...
class HeroRepositoryAlternativeImpl: HeroRepositoryAlternative {
    ...
    override suspend fun getAllHeroes(): ApiResponse<Hero> =
        ApiResponse(
            success = true,
            message = "Ok",
            result = heroes,
    ...
    override suspend fun getAllHeroes(page: Int, limit: Int): ApiResponse<Hero> =
        ApiResponse(
            success = true,
            message = "Ok",
            previousPage = calculatePage(heroes, page = page, limit = limit)[PREVIOUS_PAGE_KEY],
            nextPage = calculatePage(heroes, page = page, limit = limit)[NEXT_PAGE_KEY],
            result = provideHeroes(heroes, page = page, limit = limit),
            lastUpdated = System.currentTimeMillis()
        )
    ...
    override suspend fun searchHeroes(name: String?): ApiResponse<Hero> =
        ApiResponse(
            success = true,
            message = "Ok",
            result = findHeroes(name)
        )

    private fun calculatePage(heroes: List<Hero>, page: Int, limit: Int): Map<String, Int?> {
        val allHeroesPaginated = heroes.windowed(size = limit, step = limit, partialWindows = true)
        require(page <= allHeroesPaginated.size)

        val previousPage = if (page == 1) null else page - 1
        val nextPage = if (page == allHeroesPaginated.size) null else page + 1
        return mapOf(
            PREVIOUS_PAGE_KEY to previousPage,
    ...
    private fun provideHeroes(heroes: List<Hero>, page: Int, limit: Int): List<Hero> {
        val allHeroesPaginated = heroes.windowed(size = limit, step = limit, partialWindows = true)
        require(page > 0 && page <= allHeroesPaginated.size)
        return allHeroesPaginated[page - 1]
    }
